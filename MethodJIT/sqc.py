"""
JIT-compiled Symmetric Quasi-Classical (SQC) method.
Reference: Method/sqc.py
"""
import numpy as np
...
@njit(**_jit)
def force_sqc(qF, pF, gamma, dHij, dH0, NStates, ndof):
    F = np.empty(ndof)
    for d in range(ndof):
        F[d] = -dH0[d]
    for i in range(NStates):
        qi2 = qF[i] * qF[i] + pF[i] * pF[i] - 2.0 * gamma
        for d in range(ndof):
...
def make_sqc_kernel(hel_fn, dhel_fn, dhel0_fn):
    ...
    def run_traj(NTraj, NSteps, NStates, nskip, EStep, dtN, M_mass,
                 ...
                 map_rand,   # (NTraj, 2, NStates) -- eta_rand, theta_rand
        ...
        for itraj in prange(NTraj):
            ...
            if use_square:
                ...
                # triangle: use map_rand[itraj, 2:, :] for rejection sampling
                # Pre-generated: map_rand[itraj, 0, :] = eta values, [1,:] = theta
                for s in range(NStates):
            ...
            for step in range(NSteps):
                ...
                F1 = force_sqc(qF, pF, gamma, dHij, dH0, NStates, ndof)
                v = np.empty(ndof)
                for d in range(ndof):
                    v[d] = P[d] / M_mass
                ...
                dHij = dhel_fn(R, c)
                dH0 = dhel0_fn(R, omega)
                F2 = force_sqc(qF, pF, gamma, dHij, dH0, NStates, ndof)
                for d in range(ndof):
